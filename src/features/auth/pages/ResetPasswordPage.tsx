...
export const ResetPasswordPage = () => {
    ...
    const onSubmit = async (data: ResetPasswordFormData) => {
        ...
        try {
            ...
            toastSuccess('Password has been successfully reset. Please login.');
            navigate('/login');
        } catch (err: unknown) {
            const apiErr = err as ApiError;
            if (apiErr.field) {
                form.setError(apiErr.field as keyof ResetPasswordFormData, { message: apiErr.message });
            } else {
                setError(apiErr.message || 'An unexpected error occurred during password reset');
            }
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white relative overflow-hidden">
            ...
            <div className="max-w-md w-full mx-auto p-8 glass-card bg-white/80 dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 relative z-10 animate-fade-up shadow-xl dark:shadow-none">
                <h2 className="text-3xl font-bold text-center text-slate-900 dark:text-white mb-6">Create New Password</h2>
